...
def annotate(
	...
	def render(index):
		...
		header = render_additional_columns(box, ix, additional_columns)

		# display new example
		with out:
			clear_output(wait=True)
			image = display_fn(annotations.at[ix, example_column])
			display(
				VBox([
					header,
					image
	...
	def render_buttons(buttons=None, buttons_in_a_row=-1):
		buttons = buttons or []
	...
	def render_additional_columns(box, ix, additional_columns):
		header = HBox([
			box,
			HTML(annotations.loc[ix, additional_columns].to_frame().T.to_html(notebook=True))
		])
		return header
	...
	box = render_buttons(buttons=buttons, buttons_in_a_row=buttons_in_a_row)
	# if len(buttons) > buttons_in_a_row:
	# 	box = VBox([HBox(buttons[x:x + buttons_in_a_row])
	# 				for x in range(0, len(buttons), buttons_in_a_row)])
	# else:
